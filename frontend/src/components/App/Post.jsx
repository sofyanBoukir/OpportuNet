...
export const Post = () => {
  ...
  const handleClose = ({ postInfo }) => {
    setAnchorEl(null);
  };
  const [isExpanded, setIsExpanded] = useState(false);
  const text =
    "Theres a small issue in your class name usage. In JSX (React), class names should not start with a dot (.) inside the className attribute. ";
  const toggleText = () => {
    ...
    <div className="w-[100%] md:w-[100%] bg-white rounded-xl">
      <div className="w-[100%] px-4 py-4 justify-between flex flex-row items-center">
        <div className=" flex flex-row items-center">
          <div className="w-[15%] lg:w-[12%]">
            {" "}
            <img src="/profil.jpg" className=" rounded-full" alt="" />
          </div>
          <div className="px-3">
      ...
      <div>
        ...
        <div className="flex flex-row items-center py-2 px-5  justify-center">
          <button className="flex flex-row items-center w-[30%] hover:bg-gray-100 justify-center py-2 rounded-lg cursor-pointer">
            <div className="flex flex-row items-center gap-2">
              <HeartIcon className="text-black w-6" /> <h1>Like</h1>
            </div>
          </button>
          <button
            className="flex flex-row items-center w-[30%] hover:bg-gray-100 justify-center py-2 rounded-lg cursor-pointer"
