...
const updateInfo = async (request, response) => {
  try {
    ...
    if (
      user.profile_picture &&
      user.profile_picture !== `/users/userDefaultImage.jpg`
    ) {
...
const deleteEducation = async (request, response) => {
  try {
    ...
    const newEducations = user.education.filter(
      (education) => education._id.toString() !== educationId
    );
...
const addExperience = async (request, response) => {
  try {
    const userId = request.user.id;
    const { position, company, location, year, current, description } =
      request.body;
...
const deleteExperience = async (request, response) => {
  try {
    ...
    const newExperiences = user.experience.filter(
      (experience) => experience._id.toString() !== experienceId
    );
